...
def create_user():
    ...
    if 'username' not in data or 'email' not in data or 'password' not in data:
        return bad_request('must include username, email and password to register')
    if User.query.filter_by(username=data['username']).first():
        return bad_request("please use a different username, this one is taken")
...
def update_user(id):
    ...
    if 'email' in data and data['email'] != user.email and User.query.filter_by(email=data['email']).first():
        return bad_request("Email already in use, please use a different email")
